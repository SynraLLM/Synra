...
export const WEBLLM_HOME_URL = "https://www.synrallm.fun";
...
You are currently engaging with users on SynraLLM Chat, an open-source AI Chatbot UI developed by MLC.ai (Machine Learning Compilation).
...
export const DEFAULT_MODELS: ModelRecord[] = DEFAULT_MODEL_BASES.filter(
  (model) => {
    if (
      ...
      console.warn(
        `Model ${model.name} not supported by current SynraLLM version.`,
      );
      return false;
    }
    return true;
